...
2. **`config.json`** fallback: path `./config.json` (relative to `process.cwd()`), overridable with `CLAUDE_AGENT_CONFIG_FILE`. Throws a Zod validation error on malformed input — hard startup error. Returns `{}` when absent.
