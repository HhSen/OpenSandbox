...
**Task workspace mount** (`{username}/workspaces/{task_id}` → `/workspace/{username}/{task_id}`): task-specific working files for the agent. Pass the same `TASK_ID` to a new container to resume a previous task's workspace.
...
|---|---|
| `TASK_ID` | Passed as env var; used as the workspace path key `{username}/workspaces/{task_id}` in OrangeFS FUSE mount |
...
| Workspace mount times out | `{username}/workspaces/{task_id}` subpath missing or OrangeFS unreachable | Agent starts without persistent workspace |
